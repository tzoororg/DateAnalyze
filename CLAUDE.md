...
- **`js/feedback.js`** / **`js/feedback-config.js`** — In-app "Send feedback" modal (⋯ menu). POSTs text + optional photo to a Cloudflare Worker (`worker/feedback-worker.js`) that opens a `feedback`-labeled GitHub issue. Adding the `plan` label to an issue triggers a GitHub Action (`.github/workflows/feedback-plan.yml` + `generate_plan.py`) that comments a rewording + plan (manual label gate so the paid Claude API call only fires for items you choose to plan). The issue number is the serial referenced later as "implement feedback #N". Setup/ops: `FEEDBACK_PLAN.md`.
