...
- **`js/feedback.js`** / **`js/feedback-config.js`** — In-app "Send feedback" modal (⋯ menu). POSTs text + optional photo to a Cloudflare Worker (`worker/feedback-worker.js`) that opens a labeled GitHub issue; a GitHub Action (`.github/workflows/feedback-plan.yml` + `generate_plan.py`) auto-comments a rewording + plan. The issue number is the serial referenced later as "implement feedback #N". Setup/ops: `FEEDBACK_PLAN.md`.
