...
- **#17 & #18 — Firebase Storage "User does not have permission to access ...photos/..." (storage/unauthorized) crash**: `sync.js` `getPhoto` rethrew any Storage read error that wasn't `object-not-found`, so a transient `storage/unauthorized` (an auth-token blip on a legitimate member — the reporter was the space owner) escaped as an unhandledrejection and surfaced as a scary error report. A photo that can't be fetched right now now degrades to a missing thumbnail: unexpected Storage read failures warn and return `null` instead of throwing, mirroring `deletePhoto`'s best-effort handling.
