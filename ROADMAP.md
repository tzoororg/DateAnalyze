...
**Status.** ✅ Shipped (feedback [#16](https://github.com/tzoororg/DateAnalyze/issues/16)) — mock `design/roadmap/photo-date-autofill.html`, implemented in `onPhotoPick()` → `dateFromPhoto()` (`js/ui.js`) with the undo chip; smoke-tested (auto-fill, undo, hand-set date not clobbered).
...
- **medium** — Ideas filter panel is the app's one patch of stock Material: a native `<select>` for "Max effort" sits above the custom `Category▾`/`Vibe▾` pills, three control idioms stacked. Make effort a pill-dropdown too, and drop the `MAX BUDGET` caps heading (the Free/$/$$/$$$ pills self-explain). *(found preparing v2.4.0 release)*
- **low** — History rows have inconsistent heights because the cost badge wraps to a second line on longer titles, leaving a dangling `·`. Right-align the cost badge next to the heart rating so rows are fixed height. *(found preparing v2.4.0 release)*
- **low** — Log form category icons render as a 6+5 grid; a single horizontally-scrollable strip would reclaim a row of vertical space in the tallest view in the app. *(found preparing v2.4.0 release)*
...
- **low** — Wrapped share card truncates its own stat labels (`Creative / Cla…`, `Picnic in the p…`) — on the one artifact designed to leave the app. *(found preparing v2.4.0 release)*
