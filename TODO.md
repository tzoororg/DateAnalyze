# TODO

- [x] Discuss date widget redesign
  - Element size calibration
  - New feature: partner rating
  - New feature: comments?
  - Link from home to album?
- [x] Discuss demo mode for developing features
- [x] Discuss testing procedure during development — done: three-layer suite in test/ (see CLAUDE.md Testing)
- [med] Discuss Heroku connection
- [med] Advance roadmap items
- [x] Make the app work on iOS — icon + sign-in redirect shipped; on-device verify pending (plans/done/IOS_PLAN.md, roadmap #12)
- [x] Discuss hardening for production
  - Image storage?
  - Security?
- [low] Advance hardening for production plan
- Discuss plan usage estimations
